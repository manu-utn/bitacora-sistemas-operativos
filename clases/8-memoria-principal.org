...
** Base
   ...
   #+BEGIN_qUOTE
   ~Dirección lógica = Base (principio de la particon, dirección física) + Límite (offset)~

   <<DUDA>>: DF = Base (principio de la partición) + DL ()
...
*** Conceptos
    ...
    - Es como una *caché* pero para la *Tabla de Páginas*
    ...
    1. primer acceso a la Tabla de Páginas (TP)
    2. segundo acceso a la Dirección Física (DF)

    En cada entrada de página puede guardar tambien el identificador del proceso
